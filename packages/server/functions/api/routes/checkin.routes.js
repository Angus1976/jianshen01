...
router.get('/themes', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
        const themes = await checkin_model_1.CheckinTheme.findActiveThemes();
        // 获取用户今日在各主题的打卡次数
        const userId = req.user.userId;
        const themesWithStatus = await Promise.all(themes.map(async (theme) => {
            const todayCount = await checkin_model_1.CheckinRecord.getUserTodayCheckinCount(userId, theme.themeId);
            return {
                ...theme,
        ...
        (0, response_1.success)(res, themesWithStatus);
    }
    catch (err) {
        next(err);
    }
...
router.get('/themes/:themeId', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
        ...
        if (theme.status !== 1) {
            return (0, response_1.error)(res, shared_1.ApiCode.NotFound, '主题已下架', 404);
        }
        // 获取用户今日打卡次数
        const todayCount = await checkin_model_1.CheckinRecord.getUserTodayCheckinCount(req.user.userId, themeId);
        // 获取该主题最近的打卡记录
        const recentRecords = await checkin_model_1.CheckinRecord.findByThemeId(themeId, 10);
...
router.post('/create', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
        const { themeId, content, images = [], location } = req.body;
        if (!themeId) {
            return (0, response_1.error)(res, shared_1.ApiCode.BadRequest, '请选择打卡主题');
        }
        const theme = await checkin_model_1.CheckinTheme.findByThemeId(themeId);
        if (!theme) {
            return (0, response_1.error)(res, shared_1.ApiCode.NotFound, '主题不存在', 404);
        ...
        if (theme.status !== 1) {
            return (0, response_1.error)(res, shared_1.ApiCode.BadRequest, '该主题已下架');
        }
        // 检查今日打卡次数
        const todayCount = await checkin_model_1.CheckinRecord.getUserTodayCheckinCount(req.user.userId, themeId);
        if (todayCount >= theme.maxDailyCheckin) {
            return (0, response_1.error)(res, shared_1.ApiCode.BadRequest, `该主题每日最多打卡 ${theme.maxDailyCheckin} 次`);
        }
        ...
        // 创建打卡记录
        const record = await checkin_model_1.CheckinRecord.create({
        ...
        if (theme.rewardPoints > 0) {
            await user_model_1.User.updatePoints(user.userId, theme.rewardPoints);
        ...
        // 增加成长值
        await user_model_1.User.updateGrowthValue(user.userId, 10);
        (0, response_1.success)(res, {
...
router.get('/my-records', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
...
router.get('/records/:recordId', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
        const { recordId } = req.params;
        const record = await checkin_model_1.CheckinRecord.findByRecordId(recordId);
        if (!record) {
            return (0, response_1.error)(res, shared_1.ApiCode.NotFound, '打卡记录不存在', 404);
        }
        // 获取主题详情
        const theme = await checkin_model_1.CheckinTheme.findByThemeId(record.themeId);
        (0, response_1.success)(res, {
            ...record,
            theme,
        });
...
router.post('/records/:recordId/share', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
        const { recordId } = req.params;
        const record = await checkin_model_1.CheckinRecord.findByRecordId(recordId);
        if (!record) {
        ...
        if (record.userId !== req.user.userId) {
            return (0, response_1.error)(res, shared_1.ApiCode.Forbidden, '无权操作此记录', 403);
        }
        // 获取主题详情
        const theme = await checkin_model_1.CheckinTheme.findByThemeId(record.themeId);
        ...
        const posterData = {
            ...
            templateImages: (theme === null || theme === void 0 ? void 0 : theme.templateImages) || [],
            // 模拟的海报URL
...
router.post('/records/:recordId/share-callback', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
        const { recordId } = req.params;
        const record = await checkin_model_1.CheckinRecord.findByRecordId(recordId);
        if (!record) {
        ...
        // 标记为已分享
        await checkin_model_1.CheckinRecord.markAsShared(recordId);
        // 发放分享奖励
        let rewardPoints = 0;
        if (record.shareRewardPoints > 0) {
            ...
            const user = await user_model_1.User.findByUserId(req.user.userId);
            if (user) {
                await user_model_1.User.updatePoints(user.userId, rewardPoints);
                await points_model_1.PointsRecord.create({
                    recordId: (0, uuid_1.v4)(),
                    userId: user.userId,
                    type: 'earn',
...
router.get('/stats', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
        const userId = req.user.userId;
        // 获取用户所有打卡记录
        const records = await checkin_model_1.CheckinRecord.findByUserId(userId, 1000);
        // 计算总打卡次数
...
router.get('/calendar', auth_middleware_1.authMiddleware, async (req, res, next) => {
    try {
        ...
        // 获取指定月份的打卡记录
        const allRecords = await checkin_model_1.CheckinRecord.findByUserId(req.user.userId, 1000);
